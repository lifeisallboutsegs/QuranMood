...
async function scrapeQuranVerse(surah, verse) {
  try {
...
async function enhanceVerseWithGemini(scrapedData, surah, verse) {
  try {
    const config = {
...
`;
    ...
    return {
      ...
      arabic: scrapedData.arabic,
      english: scrapedData.english,
      bangla: "Translation not available (AI service error)",
      reference: {
        surah: parseInt(surah),
        ayah: parseInt(verse),
        text: scrapedData.surahName,
      ...
      source: scrapedData.url,
      tags: ["quran"],
      context: "Context not available (AI service error)",
      translation_info: {
        english_translator: "From source",
        bangla_translator: "Gemini AI (Error)",
        language: "multi",
      },
      created_at: new Date().toISOString(),
    };
...
async function enhanceVerseWithGeminiStream(scrapedData, surah, verse) {
  try {
    const config = {
      responseMimeType: "application/json",
      responseSchema: {
